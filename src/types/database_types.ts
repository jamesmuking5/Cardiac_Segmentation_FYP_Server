 ...
 * @enum {string}
 * @property {string} RLE_STACKING - Reconstructed by stacking 2D RLE segmentation masks into 3D NIfTI volume
 * @property {string} AI_SDF_MODEL - Generated by AI using SDF (Signed Distance Field) reconstruction model
 * @property {string} MANUAL_3D - Manually created or edited 3D reconstruction
 */
export enum ReconstructionSource {
  RLE_STACKING = "rle_stacking",     // From 2D segmentation mask stacking
  AI_SDF_MODEL = "ai_sdf_model",     // From AI 3D reconstruction model
  MANUAL_3D = "manual_3d",           // Manually created/edited
}

// Enumeration for 3D Mesh Formats
/**
...
/**
 * Defines the structure for a project's 3D reconstruction results.
 * This is a child of IProject and references back to the project it belongs to.
 * Stores the 3D reconstruction meshes and volumetric measurements for a project.
 ...
 * @property {ReconstructionSource} reconstructionSource - The source method used for 3D reconstruction.
 * @property {MeshFormat} meshFormat - The format of the 3D mesh files (obj, ply, glb, etc.).
 * @property {string} filename - Server-generated reconstruction filename (e.g., projectid_reconstructionid_3d).
 * @property {number} filesize - Total size of all reconstruction files in bytes.
 * @property {string} filehash - Combined SHA256 hash of all reconstruction files for integrity.
 * @property {string} basepath - Base path for reconstruction storage (e.g., s3://bucket/reconstructions/projectid/reconstructionid).
 * @property {string} reconstructionfolderpath - Folder where all reconstruction files are stored (e.g., basepath/).
 * @property {object} [reconstructedVolume] - Information about the 3D segmentation volume (NIfTI format).
 * @property {string} reconstructedVolume.path - S3 path to 3D NIfTI segmentation volume.
 * @property {string} reconstructedVolume.filename - Volume filename (e.g., projectid_reconstructionid_volume.nii.gz).
 * @property {number} reconstructedVolume.filesize - Size of 3D NIfTI file in bytes.
 * @property {string} reconstructedVolume.hash - SHA256 hash of 3D NIfTI for integrity.
 * @property {Date} reconstructedVolume.timestamp - When 3D NIfTI was generated.
 * @property {object} reconstructedMeshes - Information about the 3D surface meshes.
 * @property {string} reconstructedMeshes.meshfolderpath - S3 folder path where all mesh files are stored (e.g., basepath/meshes).
 * @property {object[]} reconstructedMeshes.frames - Array of frame objects for temporal cardiac data.
 * @property {number} reconstructedMeshes.frames.frameindex - The index of the cardiac frame/phase (0-based).
 * @property {boolean} reconstructedMeshes.frames.framereconstructed - Indicates if the frame has been reconstructed.
 * @property {string} reconstructedMeshes.frames.framefolderpath - S3 folder path for this specific frame (e.g., meshfolderpath/frame0).
 * @property {object[]} reconstructedMeshes.frames.components - Array of cardiac component meshes for this frame.
 * @property {ComponentBoundingBoxesClass} reconstructedMeshes.frames.components.class - The cardiac component class (rv, myo, lvc).
 * @property {string} reconstructedMeshes.frames.components.meshPath - Full S3 path to the component mesh file.
 * @property {string} reconstructedMeshes.frames.components.filename - Mesh filename (e.g., projectid_reconstructionid_frame0_rv.glb).
 * @property {number} reconstructedMeshes.frames.components.filesize - Size of mesh file in bytes.
 * @property {string} reconstructedMeshes.frames.components.hash - SHA256 hash of mesh file.
 * @property {number} reconstructedMeshes.frames.components.confidence - AI confidence score for this reconstruction (0-1).
 * @property {object} [reconstructedMeshes.frames.components.measurements] - Component-specific measurements.
 * @property {number} [reconstructedMeshes.frames.components.measurements.volume] - Component volume in mL.
 * @property {number} [reconstructedMeshes.frames.components.measurements.surfaceArea] - Component surface area in cm².
 * @property {number} [reconstructedMeshes.frames.components.measurements.wallThickness] - Average wall thickness in mm (for myo).
 * @property {object} [combinedMesh] - Combined mesh (optional) - single file containing all components.
 * @property {string} combinedMesh.path - Full S3 path to combined multi-component mesh.
 * @property {string} combinedMesh.filename - Combined mesh filename (e.g., projectid_reconstructionid_combined.glb).
 * @property {number} combinedMesh.filesize - Size of combined mesh file in bytes.
 * @property {string} combinedMesh.hash - SHA256 hash of combined mesh file.
 * @property {string} combinedMesh.folderpath - S3 folder path for combined mesh (e.g., basepath/combined).
 * @property {object} [cardiacMetrics] - Overall cardiac function measurements (optional).
 * @property {number} [cardiacMetrics.endDiastolicVolume] - Maximum LVC volume (mL).
 * @property {number} [cardiacMetrics.endSystolicVolume] - Minimum LVC volume (mL).
 * @property {number} [cardiacMetrics.heartRate] - Beats per minute (if temporal data available).
 * @property {number} [cardiacMetrics.cardiacOutput] - Heart rate × stroke volume (L/min).
 */
export interface IProjectReconstruction {
  ...
  description?: string; // Description of the 3D reconstruction
  isSaved: boolean; // Indicates if the 3D reconstruction is saved in the database
  isAIGenerated: boolean; // Indicates if the reconstruction is AI-generated (should not delete if it's AI output)
  reconstructionSource: ReconstructionSource; // Source method used for reconstruction
  meshFormat: MeshFormat; // Format of the 3D mesh files
  
  // File properties 
  filename: string; // Server-generated reconstruction filename (e.g., projectid_reconstructionid_3d)
  filesize: number; // Total size of all reconstruction files in bytes
  filehash: string; // Combined SHA256 hash of all reconstruction files for integrity
  
  // Location tracking
  basepath: string; // Base path for reconstruction storage (e.g., s3://bucket/reconstructions/projectid/reconstructionid)
  reconstructionfolderpath: string; // Folder where all reconstruction files are stored (e.g., basepath/)
  
  // 3D Segmentation Volume (NIfTI format) - from stacking 2D RLE masks or direct 3D processing
  reconstructedVolume?: {
    path: string; // S3 path to 3D NIfTI segmentation volume
    filename: string; // Volume filename (e.g., projectid_reconstructionid_volume.nii.gz)
    filesize: number; // Size of 3D NIfTI file in bytes
    hash: string; // SHA256 hash of 3D NIfTI for integrity
    timestamp: Date; // When 3D NIfTI was generated
  };
  
  // 3D Surface Meshes - organized by cardiac frames/phases with flexible S3 paths
  reconstructedMeshes: {
    meshfolderpath: string; // S3 folder path where all mesh files are stored (e.g., basepath/meshes)
    frames: {
      frameindex: number; // The index of the cardiac frame/phase (0-based)
      framereconstructed: boolean; // Indicates if the frame has been reconstructed
      framefolderpath: string; // S3 folder path for this specific frame (e.g., meshfolderpath/frame0)
      components: {
        class: ComponentBoundingBoxesClass; // Cardiac component class (rv, myo, lvc)
        meshPath: string; // Full S3 path to the component mesh file
        filename: string; // Mesh filename (e.g., projectid_reconstructionid_frame0_rv.glb)
        filesize: number; // Size of mesh file in bytes
        hash: string; // SHA256 hash of mesh file
        confidence: number; // AI confidence score for this reconstruction (0-1)
        measurements?: {
          volume?: number; // Component volume in mL
          surfaceArea?: number; // Component surface area in cm²
          wallThickness?: number; // Average wall thickness in mm (primarily for myocardium)
        };
      }[];
    }[];
  };
  
  // Combined mesh (optional) - single file containing all components with flexible output
  combinedMesh?: {
    path: string; // Full S3 path to combined multi-component mesh
    filename: string; // Combined mesh filename (e.g., projectid_reconstructionid_combined.glb)
    filesize: number; // Size of combined mesh file in bytes
    hash: string; // SHA256 hash of combined mesh file
    folderpath: string; // S3 folder path for combined mesh (e.g., basepath/combined)
  };
  
  // Overall cardiac function metrics (calculated from all frames)
  cardiacMetrics?: {
    endDiastolicVolume?: number; // Maximum LVC volume (mL)
    endSystolicVolume?: number; // Minimum LVC volume (mL)
    heartRate?: number; // Beats per minute (if temporal data available)
    cardiacOutput?: number; // Heart rate × stroke volume (L/min)
  };
}
