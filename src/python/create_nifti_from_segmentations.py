    ...
    try:
        ...
        for i in range(0, len(parts), 2):
            ...
            if start < 0 or start + length > len(mask):
                print(f"Warning: RLE segment start={start}, length={length} is out of bounds for plane size {len(mask)}. Skipping segment.", file=sys.stderr)
                continue
            mask[start:start+length] = True
    except ValueError:
        print(f"Warning: Could not parse RLE string parts into integers: '{rle_string}'. Returning empty mask for this RLE.", file=sys.stderr)
...
def create_nifti_from_segmentations(segmentations_json_path, original_nifti_path, output_nifti_path, plane_height_for_rle, plane_width_for_rle):
    try:
        ...
        if not all_segmentation_sets:
            print("Warning: No segmentation sets found in JSON. Output NIfTI will be empty.", file=sys.stderr)
        else:
            ...
            for frame_obj in segmentation_set.get("frames", []):
                ...
                if frame_idx >= num_frames_nifti:
                    print(f"Warning: Frame index {frame_idx} from JSON exceeds NIfTI frames {num_frames_nifti}. Skipping.", file=sys.stderr)
                    continue

                for slice_obj in frame_obj.get("slices", []):
                    slice_idx = slice_obj.get("sliceindex", 0)
                    if slice_idx >= num_slices_nifti:
                        print(f"Warning: Slice index {slice_idx} from JSON exceeds NIfTI slices {num_slices_nifti}. Skipping.", file=sys.stderr)
                        continue
                    
                    current_slice_label_mask = np.zeros((plane_height_for_rle, plane_width_for_rle), dtype=np.uint8)

                    for mask_entry in slice_obj.get("segmentationmasks", []):
                        class_name = str(mask_entry.get("class", "")).lower() # Ensure lowercase for map lookup
                        ...
                        try:
                            class_mask_2d = decode_rle(rle_string, plane_height_for_rle, plane_width_for_rle)
                            current_slice_label_mask[class_mask_2d] = label # Higher labels overwrite lower if overlap
                        except Exception as e_rle:
                            print(f"Error decoding RLE for class '{class_name}', frame {frame_idx}, slice {slice_idx}: {e_rle}", file=sys.stderr)
                    ...
                    if current_slice_label_mask.shape[0] != img_shape[0] or current_slice_label_mask.shape[1] != img_shape[1]:
                        print(f"Warning: Mismatch between RLE plane dimensions ({current_slice_label_mask.shape}) and NIfTI plane ({img_shape[0]}, {img_shape[1]}) for slice {slice_idx}, frame {frame_idx}. Resizing/padding might be needed or check RLE source.", file=sys.stderr)
                        # Potentially add resizing logic here if necessary, or ensure consistency upstream
                        # For now, direct assignment assumes dimensions match.
                        # If they don't, this will error or lead to incorrect results.
                        # Example: Pad or crop current_slice_label_mask to fit img_shape[0], img_shape[1]
                        # For simplicity, we'll assume they match for now.
                        pass


                    if is_4d:
                        segmentation_data[..., slice_idx, frame_idx] = current_slice_label_mask # Assumes (H, W) order for RLE plane
                    else: # 3D
                        segmentation_data[..., slice_idx] = current_slice_label_mask # Assumes (H, W) order for RLE plane
        
        new_header = original_header.copy()
        new_header.set_data_dtype(np.uint8)
        new_header['cal_max'] = float(np.max(segmentation_data)) if segmentation_data.size > 0 else 0.0
        new_header['cal_min'] = float(np.min(segmentation_data)) if segmentation_data.size > 0 else 0.0
        ...
        seg_nifti_img = nib.Nifti1Image(segmentation_data, affine, header=new_header)
        ...
        print(f"Error creating NIfTI from segmentations: {str(e)}", file=sys.stderr)
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    create_nifti_from_segmentations(segmentations_json_path_arg, original_nifti_path_arg, output_nifti_path_arg, plane_height_arg, plane_width_arg)