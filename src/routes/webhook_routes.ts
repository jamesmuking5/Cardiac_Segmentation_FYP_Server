...
import { JobStatus, IProjectSegmentationMask, ComponentBoundingBoxesClass, CRUDOperation } from "../types/database_types"; // Import JobStatus enum 
...
router.post("/api/gpu-webhook", async (req: Request, res: Response) => {
    ...
    const { status, result, error } = req.body; // Changed 'message' to 'error'
    ...
    } else if (status === 'processing') { // This case won't be hit by Python's current callback
    ...
    try {
        // Using the error field for the message property in updateJob
