    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (result.success && result.segmentationData) { // Check for segmentationData
                ...
                logger.error(`${serviceLocation}: Failed to process direct manual segmentation for project ${projectId}, image ${image_name}. Error: ${result.error || result.message}`);
                return res.status(500).json({ success: false, message: result.error || result.message || "Failed to process manual segmentation." });
            }
        } catch (error: unknown) {
